...
GeographicLock::GeographicLock(CurrentLocationStrategy* initialCurrentLocationStrategy) {
    currentLocationStrategy = initialCurrentLocationStrategy;
}

void GeographicLock::set_zones (vector<Zone> initialZones) {
    zones = initialZones;
    zoneBeingLocated = &zones[0];
}

void GeographicLock::update_state() {
...
void GeographicLock::update_distance_to_zone_being_located() {
...
bool GeographicLock::have_visited_all_zones() {
